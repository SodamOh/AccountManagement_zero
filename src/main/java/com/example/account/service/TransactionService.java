...
public class TransactionService {
     ...
     * 정책상 위반되는 케이스
     * 1. 사용자가 없는 경우
     * 2. 사용자 아아디와 계좌 소유주가 다른 경우
     * 3. 계좌가 이미 해지 상태인 경우
     * 4. 거래금액이 이전보다 큰 경우
     * 5. 거래 금액이 너무 작거나 너무 큰 경우 실패
     */
    ...
    public TransactionDto useBalance(Long userId, String accountNumber,
        ...
        //잔액 변경
        account.useBalance(amount); // 중요 정보를 바꿀때는 해당 객체의 엔티티 안에서 수정할 수 있도록해요

        // 변경된 잔고에 대한 거래 기록 남기기(account 기반으로 신규 트랜잭션 생성)
        return TransactionDto.fromEntity(saveAndGetTransaction(USE, S, account, amount));
    }
    // UUID : universally unique identifier. 중간에 2개의 하이픈이 들어가서 제거해서 깔끔하게 많이 사용.
